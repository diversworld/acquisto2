...
$GLOBALS['TL_DCA']['tl_shop_categories'] = array
(
	...
	(
		...
		(
			'fields' => array('title'),
			'format' => '%s',
			'label_callback' => array('tl_shop_categories', 'addIcon')
		),
		...
		(
			'edit' => array
			(
				'label' => &$GLOBALS['TL_LANG']['tl_shop_categories']['edit'],
				'href' => 'act=edit',
				'icon' => 'edit.gif',
//                'button_callback'     => array('tl_shop_categories', 'editPage')
			),
			'copy' => array
			(
				'label' => &$GLOBALS['TL_LANG']['tl_shop_categories']['copy'],
...
//            'copyChilds' => array
//            (
//                'label'               => &$GLOBALS['TL_LANG']['tl_shop_categories']['copyChilds'],
//                'href'                => 'act=paste&amp;mode=copy&amp;childs=1',
//                'icon'                => 'copychilds.gif',
//                'attributes'          => 'onclick="Backend.getScrollOffset();"',
//                'button_callback'     => array('tl_shop_categories', 'copyPageWithSubpages')
//            ),
			'cut' => array
			(
				'label' => &$GLOBALS['TL_LANG']['tl_shop_categories']['cut'],
			...
			'delete' => array
			(
				'label' => &$GLOBALS['TL_LANG']['tl_shop_categories']['delete'],
				'href' => 'act=delete',
				'icon' => 'delete.gif',
				'attributes' => 'onclick="if (!confirm(\'' . $GLOBALS['TL_LANG']['MSC']['deleteConfirm'] . '\')) return false; Backend.getScrollOffset();"',
				'button_callback' => array('tl_shop_categories', 'deletePage')
			),
//            'toggle' => array
//            (
//                'label'               => &$GLOBALS['TL_LANG']['tl_shop_categories']['toggle'],
//                'icon'                => 'visible.gif',
//                'attributes'          => 'onclick="Backend.getScrollOffset(); return AjaxRequest.toggleVisibility(this, %s);"',
//                'button_callback'     => array('tl_shop_categories', 'toggleIcon')
//            ),
			'show' => array
			(
				'label' => &$GLOBALS['TL_LANG']['tl_shop_categories']['show'],
				'href' => 'act=show',
				'icon' => 'show.gif'
			)
		)
	...
	(
		...
		'title' => array
		(
			'label' => &$GLOBALS['TL_LANG']['tl_shop_categories']['title'],
			'exclude' => true,
			'inputType' => 'text',
		...
		'alias' => array
		(
			'label' => &$GLOBALS['TL_LANG']['tl_shop_categories']['alias'],
			'exclude' => true,
			'inputType' => 'text',
			...
			'save_callback' => array
			(
				array('tl_shop_categories', 'generateAlias')
			)
		),
		'type' => array
		(
			'label' => &$GLOBALS['TL_LANG']['tl_shop_categories']['type'],
			'exclude' => true,
			'inputType' => 'select',
		...
		'published' => array
		(
			'label' => &$GLOBALS['TL_LANG']['tl_shop_categories']['published'],
			'default' => 1,
			'exclude' => true,
			'inputType' => 'checkbox',
		...
		'seo_keywords' => array
		(
			'label' => &$GLOBALS['TL_LANG']['tl_shop_categories']['seo_keywords'],
			'inputType' => 'textarea',
			'search' => false,
		...
		(
			'label' => &$GLOBALS['TL_LANG']['tl_shop_categories']['seo_description'],
			'inputType' => 'textarea',
			'search' => false,
			'eval' => array('style' => 'height: 60px;', 'mandatory' => false, 'tl_class' => 'clr'),
			'sql' => "text NOT NULL"
		...
		'beschreibung' => array
		(
			'label' => &$GLOBALS['TL_LANG']['tl_shop_categories']['beschreibung'],
			'inputType' => 'textarea',
			'search' => false,
		...
		'imageSrc' => array
		(
			'label' => &$GLOBALS['TL_LANG']['tl_shop_categories']['imageSrc'],
			'inputType' => 'fileTree',
			'search' => false,
		...
		'jumpTo' => array
		(
			'label' => &$GLOBALS['TL_LANG']['tl_shop_categories']['jumpTo'],
			'exclude' => true,
		...
		'jumpToPage' => array
		(
			'label' => &$GLOBALS['TL_LANG']['tl_shop_categories']['jumpToPage'],
			'exclude' => true,
			'inputType' => 'pageTree',
			'eval' => array('mandatory' => true, 'fieldType' => 'radio'),
			'sql' => "int(10) NOT NULL default '0'"
		...
		'member_groups' => array
		(
			'label' => &$GLOBALS['TL_LANG']['tl_shop_categories']['member_groups'],
			'inputType' => 'checkbox',
		...
		'cssID' => array
		(
			'label' => &$GLOBALS['TL_LANG']['tl_shop_categories']['cssID'],
			'exclude' => true,
			'inputType' => 'text',
 ...
 * @package    Controller
 */
class tl_shop_categories extends Backend
{
	...
	public function addBreadcrumb()
	{
		// Set a new node
		if (isset($_GET['node'])) {
			$this->Session->set('tl_shop_categories_node', $this->Input->get('node'));
			$this->redirect(preg_replace('/&node=[^&]*/', '', $this->Environment->request));
		}

		$intNode = $this->Session->get('tl_shop_categories_node');
		...
		if ($intNode) {
			...
			do {
				$objPage = $this->Database->prepare("SELECT * FROM tl_shop_categories WHERE id=?")
					->limit(1)
					->execute($intId);
				...
				if ($objPage->numRows < 1) {
					...
					if ($intId == $intNode) {
						$this->Session->set('tl_shop_categories_node', 0);
						return;
					}
		...
		if (!$this->User->isAdmin && !$this->User->hasAccess($arrIds, 'pagemounts')) {
			...
			$this->log('Page ID ' . $intNode . ' was not mounted', 'tl_shop_categories addBreadcrumb', TL_ERROR);
			$this->redirect('contao/main.php?act=error');
		}

		// Limit tree
		$GLOBALS['TL_DCA']['tl_shop_categories']['list']['sorting']['root'] = array($intNode);
		...
		$GLOBALS['TL_DCA']['tl_shop_categories']['list']['sorting']['breadcrumb'] .= '
  ...
  <li>' . implode(' &gt; </li><li>', $arrLinks) . '</li>
	...
	public function setDefaultLanguage()
	{
		...
		if ($this->Input->get('pid') == 0) {
			$GLOBALS['TL_DCA']['tl_shop_categories']['fields']['language']['default'] = $GLOBALS['TL_LANGUAGE'];
		} else {
			$objPage = $this->getPageDetails($this->Input->get('pid'));
			$GLOBALS['TL_DCA']['tl_shop_categories']['fields']['language']['default'] = $objPage->rootLanguage;
		}
	}
	...
	public function generateAlias($varValue, DataContainer $dc)
	{
		...
		$objAlias = $this->Database->prepare("SELECT id FROM tl_shop_categories WHERE id=? OR alias=?")
		...
		if ($objAlias->numRows > 1) {
			...
			while ($objAlias->next()) {
				...
				do {
					$objParentPage = $this->Database->prepare("SELECT id, pid, alias, type, dns FROM tl_shop_categories WHERE id=?")
						->limit(1)
						->execute($_pid);
	...
	public function copyPageWithSubpages($row, $href, $label, $title, $icon, $attributes, $table)
	{
		...
		$objSubpages = $this->Database->prepare("SELECT * FROM tl_shop_categories WHERE pid=?")
			->limit(1)
			->execute($row['id']);
	...
	public function toggleIcon($row, $href, $label, $title, $icon, $attributes)
	{
		...
		if (!$this->User->isAdmin && !$this->User->hasAccess('tl_shop_categories::published', 'alexf')) {
			return '';
		}
	...
	public function toggleVisibility($intId, $blnVisible)
	{
		...
		if (!$this->User->isAdmin && !$this->User->hasAccess('tl_shop_categories::published', 'alexf')) {
			$this->log('Not enough permissions to publish/unpublish page ID "' . $intId . '"', 'tl_shop_categories toggleVisibility', TL_ERROR);
			$this->redirect('contao/main.php?act=error');
		}

		$this->createInitialVersion('tl_shop_categories', $intId);

		// Trigger the save_callback
		if (is_array($GLOBALS['TL_DCA']['tl_shop_categories']['fields']['published']['save_callback'])) {
			foreach ($GLOBALS['TL_DCA']['tl_shop_categories']['fields']['published']['save_callback'] as $callback) {
		...
		// Update the database
		$this->Database->prepare("UPDATE tl_shop_categories SET tstamp=" . time() . ", published='" . ($blnVisible ? 1 : '') . "' WHERE id=?")
			->execute($intId);

		$this->createNewVersion('tl_shop_categories', $intId);
	}
}
